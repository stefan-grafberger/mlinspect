...
mod_train = pd.read_csv(train_file, na_values='?', index_col=0)
...
mod_train = mod_train[(mod_train['days_b_screening_arrest'] <= 30) & (mod_train['days_b_screening_arrest'] >= -30)]
# We coded the recidivist flag – is_recid – to be -1 if we could not find a compas case at all.
mod_train = mod_train[mod_train['is_recid'] != -1]
...
mod_train = mod_train[mod_train['c_charge_degree'] != "O"]
...
mod_train = mod_train[mod_train['score_text'] != 'N/A']

mod_train = mod_train.replace('Medium', "Low")
test = test.replace('Medium', "Low")

train_labels = label_binarize(mod_train['score_text'], classes=['High', 'Low'])
...
compas_pipeline.fit(mod_train, train_labels.ravel())
