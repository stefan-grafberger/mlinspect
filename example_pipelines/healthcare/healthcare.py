...
from example_pipelines.healthcare.healthcare_utils import MyW2VTransformer, \
	MyKerasClassifier, create_model
from mlinspect.utils import get_project_root

patients = pd.read_csv("{}/example_pipelines/healthcare/patients.csv"\
                       .format(get_project_root()), na_values='?')
histories = pd.read_csv("{}/example_pipelines/healthcare/histories.csv"\
                        .format(get_project_root()), na_values='?')
...
data = data[['smoker', 'last_name', 'county', 'num_children', 'race', 'income',
             'label']]
# Add 'county1' to fix NoBiasIntroducedFor issue
...
neural_net = MyKerasClassifier(build_fn=create_model, epochs=10, batch_size=1,
                               verbose=0)
pipeline = Pipeline([
