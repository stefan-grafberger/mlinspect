...
from mlinspect.backends._pandas_backend import execute_inspection_visits_data_source, PandasBackend
...
def get_input_info(df_object, caller_filename, lineno, function_info, optional_code_reference, optional_source_code) \
    ...
    if hasattr(df_object, "_mlinspect_annotation"):
        ...
        if len(singleton.inspections) == 1 and isinstance(singleton.inspections[0], RowLineage) \
            ...
            backend_result = PandasBackend.lineage_only_after_call_data_source_groupby_agg(operator_context, df_object)
        else:
